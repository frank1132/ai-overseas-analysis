...
plt.title('变量相关性热力图', fontsize=12)
st.pyplot(fig)
...
plt.colorbar(scatter, label='增长率 (%)')

# 添加阶段标签
for stage in df['发展阶段'].unique():
    mask = df['发展阶段'] == stage
    ax2.scatter(components[mask, 0], components[mask, 1], 
               label=stage, alpha=0.7)

ax2.set_xlabel(f"第一主成分 (解释方差: {pca.explained_variance_ratio_[0]*100:.2f}%)", fontsize=10)
ax2.set_ylabel(f"第二主成分 (解释方差: {pca.explained_variance_ratio_[1]*100:.2f}%)", fontsize=10)
ax2.set_title("PCA 降维结果与发展阶段分布", fontsize=12)
